 ...
 * itself until none of the rules more apply. Having applied all the rules to an element, it will have a style attribute
 * with one or more properties. Other rules strip the element they apply to, replacing it by style properties on the
 * contents, e.g. <dir> <li>
 * <p>
 * ...</li> </dir>. <p style="margin-left 1em"> ... These rules are applied to an element before processing its content
 * and replace the current element by the first element in the exposed content. After applying both sets of rules, you
 * can replace the style attribute by a class value and style rule in the document head. To support this, an association
 * of styles and class names is built. A naive approach is to rely on string matching to test when two property lists
 * are the same. A better approach would be to first sort the properties before matching.
 * 
...
final class Clean {
  ...
  private String gensymClass (Lexer lexer, String tag) {
    ...
    str = lexer.configuration.cssPrefix == null ? lexer.configuration.cssPrefix + this.classNum : "c" + this.classNum;
    this.classNum++;
    return str;
  }
  ...
  /**
   * Find style attribute in node, and replace it by corresponding class attribute. Search for class in style dictionary
   * otherwise gensym new class and add to dictionary. Assumes that node doesn't have a class attribute.
   * 
   * @param lexer Lexer
   ...
   * 
   * @param node font tag
   * @param attributeValues attribute list for node
   */
  private void addFontStyles (final Node node, final AttVal attributeValues) {
    AttVal av = attributeValues;
    while (av != null) {
   ...
   * <code>&lt;dir> &lt;li></code> to <code>&lt;div></code> with indent. The clean up rules use the pnode argument to
   * return the next node when the original node has been deleted.
   * 
   * @param lexer Lexer
   * @param node dir tag
   ...
   */
  private boolean center2Div (final Lexer lexer, final Node node, final Node[] pnode) {
    Node n = node;
    if (n.tag == this.tt.tagCenter) {
      if (lexer.configuration.dropFontTags) {
        if (n.content != null) {
          Node last = n.last;
          Node parent = n.parent;

          discardContainer(n, pnode);

          n = lexer.inferredTag("br");
      ...
      n.tag = this.tt.tagDiv;
      n.element = "div";
      addStyleProperty(n, "text-align: center");
      return true;
    }
  ...
  /**
   * Symptom: <code>&lt;div>&lt;div>...&lt;/div>&lt;/div></code> Action: merge the two divs. This is useful after nested
   * &lt;dir>s used by Word for indenting have been converted to &lt;div>s.
   * 
   * @param lexer Lexer
   ...
   */
  private boolean nestedList (final Lexer lexer, final Node node, final Node[] pnode) {
    Node n = node;
    if (n.tag == this.tt.tagUl || n.tag == this.tt.tagOl) {
      Node child = n.content;
  ...
  /**
   * Replace font elements by span elements, deleting the font element's attributes and replacing them by a single style
   * attribute.
   * 
   * @param lexer Lexer
   * @param node font tag
   ...
   */
  private Node cleanNode (final Lexer lexer, final Node node) {
    Node n = node;
    Node next = null;
    Node[] o = new Node[1];
    boolean b = false;

    for (next = n; n != null && n.isElement(); n = next) {
      o[0] = next;
      ...
      b = nestedList(lexer, n, o);
      next = o[0];
      if (b) {
        return next;
      }

      b = center2Div(lexer, n, o);
      next = o[0];
      if (b) {
        continue;
      }
      ...
      b = blockStyle(lexer, n);
      next = o[0];
      if (b) {
        continue;
      }

      b = inlineStyle(lexer, n, o);
      next = o[0];
      if (b) {
        continue;
      }

      b = font2Span(lexer, n, o);
      next = o[0];
      if (b) {
        continue;
      }
  ...
  /**
   * Special case: if the current node is destroyed by CleanNode() lower in the tree, this node and its parent no longer
   * exist. So we must jump back up the CreateStyleProperties() call stack until we have a valid node reference.
   * 
   * @param lexer Lexer
   * @param node Node
   ...
   * @param doc root node
   */
  public void cleanTree (final Lexer lexer, final Node doc) {
    final Node newDoc = createStyleProperties(lexer, doc, new Node[] { doc });
    if (!lexer.configuration.makeClean) {
      defineStyleRules(lexer, newDoc);
      createStyleElement(lexer, newDoc);
    }
  }
   ...
   * @param node root Node
   */
  public void nestedEmphasis (final Node node) {
    Node n = node;
    Node[] o = new Node[1];
    Node next;
    ...
    while (n != null) {
      ...
      if ((n.tag == this.tt.tagB || n.tag == this.tt.tagI) && n.parent != null && n.parent.tag == n.tag) {
        // strip redundant inner element
        o[0] = next;
        discardContainer(n, o);
        next = o[0];
        n = next;
        continue;
      }
   ...
   * @param node root Node
   */
  public void emFromI (final Node node) {
    Node n = node;
    while (n != null) {
      if (n.tag == this.tt.tagI) {
        n.element = this.tt.tagEm.name;
        n.tag = this.tt.tagEm;
      } else if (n.tag == this.tt.tagB) {
        n.element = this.tt.tagStrong.name;
        n.tag = this.tt.tagStrong;
      }
   ...
   * @param node root Node
   */
  public void list2BQ (final Node node) {
    Node n = node;
    while (n != null) {
      if (n.content != null) {
        list2BQ(n.content);
      ...
      if (n.tag != null && n.tag.getParser() == ParserImpl.LIST && n.hasOneChild() && n.content.implicit) {
        stripOnlyChild(n);
        n.element = this.tt.tagBlockquote.name;
        n.tag = this.tt.tagBlockquote;
        n.implicit = true;
      }
  ...
  /**
   * Replace implicit blockquote by div with an indent taking care to reduce nested blockquotes to a single div with the
   * indent set to match the nesting depth.
   * 
   * @param node root Node
   */
  public void bQ2Div (Node node) {
    ...
    while (n != null) {
      if (n.tag == this.tt.tagBlockquote && n.implicit) {
        indent = 1;

        while (n.hasOneChild() && n.content.tag == this.tt.tagBlockquote && n.implicit) {
          ++indent;
          stripOnlyChild(n);
        }
        ...
        n.element = this.tt.tagDiv.name;
        n.tag = this.tt.tagDiv;

        attval = n.getAttrByName("style");

        if (attval != null && attval.value != null) {
          attval.value = indentBuf + "; " + attval.value;
        } else {
          n.addAttribute("style", indentBuf);
        }
   ...
   * @return cleaned up Node
   */
  public Node pruneSection (final Lexer lexer, final Node node) {
    Node n = node;
    for (;;) {
      ...
      n = Node.discardElement(n);

      if (n == null) {
        return null;
      }

      if (n.type == Node.SECTION_TAG) {
        if ((TidyUtils.getString(n.textarray, n.start, 2)).equals("if")) {
          n = pruneSection(lexer, n);
          continue;
        }

        if ((TidyUtils.getString(n.textarray, n.start, 5)).equals("endif")) {
          n = Node.discardElement(n);
          break;
        }
      }
   ...
   */
  public Node stripSpan (Lexer lexer, Node span) throws SAXException {
    Node node;
    Node prev = null;
    Node content;
   ...
   * @throws SAXException in case of errors
   */
  public void cleanWord2000 (Lexer lexer, Node node) throws SAXException {
