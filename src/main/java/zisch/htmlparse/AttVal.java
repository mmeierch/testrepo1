 ...
 * @author Zisch
 */
final class AttVal implements Cloneable {
  ...
  /**
   * Check the attribute value for uppercase letters (only if the value should be lowercase, required for literal values
   * in xhtml).
   * 
   * @param lexer Lexer
   ...
   */
  void checkLowerCaseAttrValue (Lexer lexer, Node node) throws SAXException {
    if (this.value == null) {
      return;
    }
   ...
   * @throws SAXException in case of errors
   */
  public Attribute checkAttribute (Lexer lexer, Node node) throws SAXException {
