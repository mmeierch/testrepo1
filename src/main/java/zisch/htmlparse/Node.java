...
final class Node {
   ...
   */
  public void checkAttributes (final Lexer lexer) throws SAXException {
    AttVal attval;
    ...
    for (attval = this.attributes; attval != null;) {
      if (attval.asp == null && attval.php == null) {
        ...
        for (current = attval.next; current != null;) {
          if (current.asp == null && current.php == null && attval.attribute != null
            ...
            if ("class".equalsIgnoreCase(current.attribute) && lexer.configuration.joinClasses) {
              ...
              int endPos = current.value.length() - 1;

              if (current.value.charAt(endPos) == ';') {
                // attribute ends with declaration seperator
                current.value = current.value + " " + attval.value;
              } else if (current.value.charAt(endPos) == '}') {
  ...
  public void removeAttribute (AttVal attr) {
    ...
    for (av = this.attributes; av != null; av = nextVal) {
      nextVal = av.next;
  ...
  public void insertNodeAfterElement (Node node) {
    ...
    if (parentNode != null && parentNode.last == this) {
   ...
   * @throws SAXException in case of errors
   */
  public static void trimEmptyElement (Lexer lexer, Node element) throws SAXException {
   ...
   * @throws SAXException in case of errors
   */
  public static void trimTrailingSpace (Lexer lexer, Node element, Node last) throws SAXException {
    byte c;
    TagTable tt = lexer.configuration.tt;
  ...
  public static void trimInitialSpace (Lexer lexer, Node element, Node text) {
    ...
    if (text.type == TEXT_NODE && text.textarray[text.start] == (byte) ' ' && (text.start < text.end)) {
      if (TidyUtils.toBoolean(element.tag.model & Dict.CM_INLINE)
              && !TidyUtils.toBoolean(element.tag.model & Dict.CM_FIELD) && element.parent.content != element) {
        prev = element.prev;
   ...
   * @throws SAXException in case of errors
   */
  public static void trimSpaces (Lexer lexer, Node element) throws SAXException {
   ...
   * 
   * @param descTag descendant tag
   * @return <code>true</code> if node is contained in tag
   */
  public boolean isDescendantOf (Dict descTag) {
    Node parentNode;

    for (parentNode = this.parent; parentNode != null; parentNode = parentNode.parent) {
      if (parentNode.tag == descTag) {
        return true;
      }
    }
   ...
   * @throws SAXException in case of errors
   */
  public static void insertDocType (final Lexer lexer, final Node element, final Node doctype) throws SAXException {
    TagTable tt = lexer.configuration.tt;
    lexer.report.warning(lexer, element, doctype, Report.DOCTYPE_AFTER_TAGS);
    Node el = element;
    while (el.tag != tt.tagHtml) {
      el = el.parent;
    }
    insertNodeBeforeElement(el, doctype);
  }
   ...
   * @throws SAXException in case of errors
   */
  public static void fixEmptyRow (Lexer lexer, Node row) throws SAXException {
    Node cell;
   ...
   * @throws SAXException in case of errors
   */
  public static void coerceNode (Lexer lexer, Node node, Dict tag) throws SAXException {
  ...
  public static boolean insertMisc (Node element, Node node) {
    if (node.type == COMMENT_TAG || node.type == PROC_INS_TAG || node.type == CDATA_TAG || node.type == SECTION_TAG
            || node.type == ASP_TAG || node.type == JSTE_TAG || node.type == PHP_TAG || node.type == XML_DECL) {
  ...
  /**
   * TODO [javadoc]: method findTITLE
   * 
   * @param tt TODO [javadoc]
   * @return TODO [javadoc]
   */
  ...
  org.w3c.dom.Document getDocumentAdapter (final String namespaceUri) {
    if (this.type != ROOT_NODE) {
      throw new UnsupportedOperationException("Use getAdapter() to get an adapter for non-Document nodes.");
    }
    return new DOMDocumentImpl(this, namespaceUri);
  }
      ...
      switch (this.type) {
        case ROOT_NODE:
          throw new UnsupportedOperationException(
                  "Use getDocumentAdapter(String) to get an adapter for a complete Document.");
        case START_TAG:
        case START_END_TAG:
