...
final class PPrint {
  ...
  public static int getUTF8 (byte[] str, int start, int[] ch) {
    ...
    boolean err = EncodingUtils.decodeUTF8BytesToChar(n, TidyUtils.toUnsigned(str[start]), successorBytes, null, bytes,
            start + 1);
  ...
  private void printChar (int c, short mode) {
    ...
    if ("UTF8".equals(this.configuration.getOutCharEncodingName())) {
      ...
      if ((c >= 0x2000) && !TidyUtils.toBoolean(mode & PREFORMATTED)) {
        if (((c >= 0x2000) && (c <= 0x2006)) || ((c >= 0x2008) && (c <= 0x2010)) || ((c >= 0x2011) && (c <= 0x2046))
                || ((c >= 0x207D) && (c <= 0x207E)) || ((c >= 0x208D) && (c <= 0x208E))
                || ((c >= 0x2329) && (c <= 0x232A)) || ((c >= 0x3001) && (c <= 0x3003))
                || ((c >= 0x3008) && (c <= 0x3011)) || ((c >= 0x3014) && (c <= 0x301F))
                || ((c >= 0xFD3E) && (c <= 0xFD3F)) || ((c >= 0xFE30) && (c <= 0xFE44))
                || ((c >= 0xFE49) && (c <= 0xFE52)) || ((c >= 0xFE54) && (c <= 0xFE61))
                || ((c >= 0xFE6A) && (c <= 0xFE6B)) || ((c >= 0xFF01) && (c <= 0xFF03))
                || ((c >= 0xFF05) && (c <= 0xFF0A)) || ((c >= 0xFF0C) && (c <= 0xFF0F))
                || ((c >= 0xFF1A) && (c <= 0xFF1B)) || ((c >= 0xFF1F) && (c <= 0xFF20))
                || ((c >= 0xFF3B) && (c <= 0xFF3D)) || ((c >= 0xFF61) && (c <= 0xFF65))) {
  ...
  private void printAttribute (Out fout, int indent, Node node, AttVal attr) {
    ...
    for (int i = 0; i < name.length(); i++) {
      addC(TidyUtils.foldCase(name.charAt(i), this.configuration.upperCaseAttrs, this.configuration.xmlTags), linelen++);
    }
  ...
  private void printTag (Lexer lexer, Out fout, short mode, int indent, Node node) {
    ...
    for (int i = 0; i < p.length(); i++) {
      addC(TidyUtils.foldCase(p.charAt(i), this.configuration.upperCaseTags, this.configuration.xmlTags), linelen++);
    }
  ...
  private int textEndsWithNewline (Lexer lexer, Node node) {
    if (node.type == Node.TEXT_NODE && node.end > node.start) {
      ...
      while (ix >= node.start && TidyUtils.toBoolean(ch = (node.textarray[ix] & 0xff))
              && (ch == ' ' || ch == '\t' || ch == '\r')) {
        --ix;
      }
    ...
    if (node.type == Node.TEXT_NODE || (node.type == Node.CDATA_TAG && lexer.configuration.escapeCdata)) {
      ...
      if (node.tag != null && node.tag.getParser() == ParserImpl.PRE) {
        ...
        if (!this.configuration.hideEndTags
          ...
          if (shouldIndent(node)) {
            condFlushLine(fout, indent);
          } else if (TidyUtils.toBoolean(node.tag.model & Dict.CM_HTML) || node.tag == tt.tagNoframes
        ...
        if (node.tag == tt.tagBody && this.configuration.burstSlides) {
          printSlide(fout, mode, (this.configuration.indentContent ? indent + this.configuration.spaces : indent),
                  lexer);
        } else {
          last = null;
          ...
          for (content = node.content; content != null; content = content.next) {
            ...
            if (last != null && !this.configuration.indentContent && last.type == Node.TEXT_NODE && content.tag != null
        ...
        if (shouldIndent(node) || ((TidyUtils.toBoolean(node.tag.model & Dict.CM_HTML) || node.tag == tt.tagNoframes || //
                (TidyUtils.toBoolean(node.tag.model & Dict.CM_HEAD) && !(node.tag == tt.tagTitle))) && //
  ...
  public void printXMLTree (Out fout, short mode, int indent, Lexer lexer, Node node) {
    ...
    if (node.type == Node.TEXT_NODE || (node.type == Node.CDATA_TAG && lexer.configuration.escapeCdata)) {
      ...
      printPhp(fout, indent, node);
    } else if (TidyUtils.toBoolean(node.tag.model & Dict.CM_EMPTY) || node.type == Node.START_END_TAG
            && !configuration.xHTML) {
      condFlushLine(fout, indent);
      printTag(lexer, fout, mode, indent, node);
  ...
  /**
   * Called from printTree to print the content of a slide from the node slidecontent. On return slidecontent points to
   * the node starting the next slide or null. The variables slide and count are used to customise the navigation bar.
   * 
   * @param fout
   * @param mode
   * @param indent
  ...
  public void printSlide (Out fout, short mode, int indent, Lexer lexer) {
    ...
    String s;
    s = "<div onclick=\"document.location='slide" + numberFormat.format(slide < count ? slide + 1 : 1) + ".html'\">";
    // #427666 - fix by Eric Rossen 02 Aug 00
    printString(s);
    condFlushLine(fout, indent);
    ...
    if (slidecontent != null && slidecontent.tag == tt.tagH2) {
      ...
      printTree(fout, mode, (this.configuration.indentContent ? indent + this.configuration.spaces : indent), lexer,
              slidecontent);
