...
public class Lexer {
  ...
  public void parseEntity (final short mode) throws SAXException {
    ...
    while ((c = this.in.readChar()) != StreamIn.END_OF_STREAM) {
      ...
      if (first && c == '#') {
        // #431953 - start RJ
        if (!this.configuration.ncr || "BIG5".equals(this.configuration.getInCharEncodingName())
  ...
  void addStringLiteralLen (String str, int len) {
    final int strlen = Math.max(len, str.length());
   ...
   * @throws SAXException in case of errors
   */
  public String htmlVersionName () throws SAXException {
    short guessed;
    int j;
   ...
   */
  public boolean addGenerator (final Node rootNode) {
    AttVal attval;
    Node node;
    Node head = rootNode.findHEAD(this.configuration.tt);
   ...
   * 
   * @param doctypeNode doctype node
   * @return true if doctype keywords are all uppercase
   */
  public boolean checkDocTypeKeyWords (final Node doctypeNode) {
    int len = doctypeNode.end - doctypeNode.start;
    String s = TidyUtils.getString(this.lexbuf, doctypeNode.start, len);

    return !(TidyUtils.findBadSubString("SYSTEM", s, s.length()) || TidyUtils.findBadSubString("PUBLIC", s, s.length())
            || TidyUtils.findBadSubString("//DTD", s, s.length()) || TidyUtils.findBadSubString("//W3C", s, s.length()) || TidyUtils
            .findBadSubString("//EN", s, s.length()));
  }
   ...
   * @throws SAXException in case of errors
   */
  public short findGivenVersion (final Node doctypeNode) throws SAXException {
    String p, s;
    int i, j;
    int len;
    String str1;
    ...
    str1 = TidyUtils.getString(this.lexbuf, doctypeNode.start, 5);
    if (!"html ".equalsIgnoreCase(str1)) {
      return 0;
    }

    if (!checkDocTypeKeyWords(doctypeNode)) {
      report.warning(this, doctypeNode, null, Report.DTYPE_NOT_UPPER_CASE);
    }

    // give up if all we are given is the system id for the doctype
    str1 = TidyUtils.getString(this.lexbuf, doctypeNode.start + 5, 7);
    if ("SYSTEM ".equalsIgnoreCase(str1)) {
      // but at least ensure the case is correct
      if (!str1.substring(0, 6).equals("SYSTEM")) {
        System.arraycopy(TidyUtils.getBytes("SYSTEM"), 0, this.lexbuf, doctypeNode.start + 5, 6);
      }
    ...
    if ("PUBLIC ".equalsIgnoreCase(str1)) {
      if (!str1.substring(0, 6).equals("PUBLIC")) {
        System.arraycopy(TidyUtils.getBytes("PUBLIC "), 0, this.lexbuf, doctypeNode.start + 5, 6);
      }
    } else {
      this.badDoctype = true;
    }

    for (i = doctypeNode.start; i < doctypeNode.end; ++i) {
      if (this.lexbuf[i] == (byte) '"') {
        ...
        if (str1.equals("-//W3C//DTD ")) {
          ...
          for (j = i + 13; j < doctypeNode.end && this.lexbuf[j] != (byte) '/'; ++j) {
            //
          }
          len = j - i - 13;
          ...
          for (j = i + 14; j < doctypeNode.end && this.lexbuf[j] != (byte) '/'; ++j) {
            //
          }
          len = j - i - 14;
   ...
   * @throws SAXException in case of errors
   */
  public void fixHTMLNameSpace (final Node rootNode, final String profile) throws SAXException {
    Node node;
    AttVal attr;
   ...
   */
  Node newXhtmlDocTypeNode (final Node rootNode) {
    Node html = rootNode.findHTML(this.configuration.tt);
    if (html == null) {
      return null;
    }
    ...
    newdoctype.parent = rootNode;
    newdoctype.prev = null;

    if (html == rootNode.content) {
      // No <?xml ... ?> declaration.
      rootNode.content.prev = newdoctype;
      rootNode.content = newdoctype;
      newdoctype.prev = null;
    } else {
   ...
   * @throws SAXException in case of errors
   */
  public boolean setXHTMLDocType (Node root) throws SAXException {
    String fpi = " ";
    String sysid = "";
    ...
    if (this.configuration.docTypeMode == Configuration.DOCTYPE_OMIT) {
      if (doctypeNode != null) {
        Node.discardElement(doctypeNode);
      }
      return true;
    }
    ...
    if (this.configuration.docTypeMode == Configuration.DOCTYPE_AUTO) {
      ...
      if (TidyUtils.toBoolean(this.versions & Dict.VERS_HTML40_STRICT)) {
        ...
        fpi = null;
        sysid = "";
        if (doctypeNode != null)// #473490 - fix by Bjšrn Hšhrmann 10 Oct 01
        {
          Node.discardElement(doctypeNode);
        }
      }
    ...
    if (doctypeNode != null) {
      ...
      if (configuration.xHTML || configuration.xmlOut) {
        ...
        int len = doctypeNode.end - doctypeNode.start + 1;
        String start = TidyUtils.getString(this.lexbuf, doctypeNode.start, len);
      ...
      if ((doctypeNode = newXhtmlDocTypeNode(root)) == null) {
        return false;
      }
    }
    ...
    doctypeNode.textarray = new byte[length];

    System.arraycopy(this.lexbuf, this.txtstart, doctypeNode.textarray, 0, length);
    doctypeNode.start = 0;
    doctypeNode.end = length;
   ...
   * @throws SAXException in case of errors
   */
  public short apparentVersion () throws SAXException {
    switch (this.doctype) {
   ...
   * @throws SAXException in case of errors
   */
  public boolean fixDocType (Node rootNode) throws SAXException {
    Node doctypeNode;
    ...
    doctypeNode = rootNode.findDocType();
    ...
    if (this.configuration.xmlOut || this.configuration.xmlTags || this.isvoyager) {
      ...
      fixHTMLNameSpace(rootNode, XHTML_NAMESPACE);
    ...
    if (doctypeNode == null) {
      if ((doctypeNode = newXhtmlDocTypeNode(rootNode)) == null) {
        return false;
      }
    }
  ...
  /**
   * Ensure XML document starts with <code>&lt;?XML version="1.0"?&gt;</code>. Add encoding attribute if not using ASCII
   * or UTF-8 output.
   * 
   * @param rootNode root node
   * @return always true
   */
  public boolean fixXmlDecl (final Node rootNode) {
    Node xml;
    AttVal version;
    AttVal encoding;

    if (rootNode.content != null && rootNode.content.type == Node.XML_DECL) {
      xml = rootNode.content;
    } else {
      xml = newNode(Node.XML_DECL, this.lexbuf, 0, 0);
      xml.next = rootNode.content;

      if (rootNode.content != null) {
        rootNode.content.prev = xml;
        xml.next = rootNode.content;
      }

      rootNode.content = xml;
    }
   ...
   * @throws SAXException in case of errors
   */
  public Node getCDATA (Node container) throws SAXException {
    int start = 0;
    int nested = 0;
    int st = CDATA_INTERMEDIATE;
    int c;
    ...
    while ((c = this.in.readChar()) != StreamIn.END_OF_STREAM) {
      ...
      if (st == CDATA_INTERMEDIATE) {
        ...
        st = CDATA_INTERMEDIATE;
      } else if (st == CDATA_ENDTAG) {
        /* '<' + '/' + Letter found */
        if (TidyUtils.isLetter((char) c)) {
          continue;
        }
      ...
      switch (this.state) {
          ...
          if (c == '/') {
            ...
            if (TidyUtils.isLetter((char) c)) {
              ...
              if (this.txtend > this.txtstart) {
                ...
                if (m == IGNORE_WHITESPACE && this.lexbuf[this.lexsize - 1] == (byte) ' ') {
                  this.lexsize -= 1;
                  this.txtend = this.lexsize;
                }
        ...
        case LEX_ENDTAG:
          ...
          this.token = newNode(Node.END_TAG, // create endtag token
                  this.lexbuf, this.txtstart, this.txtend, TidyUtils.getString(this.lexbuf, this.txtstart, this.txtend
                          - this.txtstart));
        ...
        case LEX_STARTTAG:
          ...
          isempty[0] = false;
          attributes = null;
          this.token = newNode((isempty[0] ? Node.START_END_TAG : Node.START_TAG), this.lexbuf, this.txtstart,
                  this.txtend, TidyUtils.getString(this.lexbuf, this.txtstart, this.txtend - this.txtstart));
          ...
          (m != PREFORMATTED || preContent(this.token))
   ...
   * dealing with ASP This is a workaround for attributes, but won't deal with the case where the ASP is used to tailor
   * the attribute value. Here is an example of a work around for using ASP in attribute values: <code>href='<%=rsSchool.Fields("ID").Value%>'</code> where the
   * ASP that generates the attribute value is masked from Tidy by the quotemarks.
   * 
   * @return parsed Node
   */
   ...
   * @throws SAXException in case of errors
   */
  public String parseAttribute (boolean[] isempty, Node[] asp, Node[] php) throws SAXException {
    int start = 0;
    String attr;
    int c = 0;
    int lastc = 0;
   ...
   * @throws SAXException in case of errors
   */
  public int parseServerInstruction () throws SAXException {
    int c, delim = '"';
   ...
   * @throws SAXException in case of errors
   */
  public String parseValue (String name, boolean foldCase, boolean[] isempty, int[] pdelim) throws SAXException {
    ...
    if (quotewarning > 10 && seenGt && munge) {
      ...
      if (!AttributeTable.getDefaultAttributeTable().isScript(name)
              && !(AttributeTable.getDefaultAttributeTable().isUrl(name) && "javascript:".equals(TidyUtils.getString(
                      this.lexbuf, start, 11))) && !"<xml ".equals(TidyUtils.getString(this.lexbuf, start, 5))) // #500236
      // - fix
      // by
      // Klaus
      // Johannes
   ...
   * @throws SAXException in case of errors
   */
  public AttVal parseAttrs (boolean[] isempty) throws SAXException {
    AttVal av, list;
   ...
   * will be the case in: <code>&lt;i>&lt;h1>italic heading&lt;/h1>&lt;/i></code> which is then treated as equivalent to
   * <code>&lt;h1>&lt;i>italic heading&lt;/i>&lt;/h1></code> This is implemented by setting the lexer into a mode where
   * it gets tokens from the inline stack rather than from the input stream.
   * 
  ...
  /**
   * @return the {@link Node} of the inserted token
   */
   ...
   * @throws SAXException in case of errors
   */
  public void fixId (Node node) throws SAXException {
  ...
  /**
   * Constraint the html version in the document to the given one. Everything is allowed in proprietary version of HTML
  ...
  protected boolean preContent (Node node) {
    ...
    if (node.tag == null || node.tag == this.configuration.tt.tagP
